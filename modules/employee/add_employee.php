...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // --- Image field validation and upload ---
    $image_file_name = '';
    if (isset($_FILES['image']) && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) {
        ...
        if ($img['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($img_ext, $allowed)) {
                ...
                $image_file_name = 'emp_' . time() . '_' . rand(100,999) . '.' . $img_ext;
    ...
    if (!$errors) {
        $stmt = $conn->prepare("INSERT INTO employee_info 
            (employee_name, designation, unit_id, email, phone, address, gender, date_of_birth, nid, join_date, status, image)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssisssssssss", $employee_name, $designation, $unit_id, $email, $phone, $address, $gender, $date_of_birth, $nid, $join_date, $status, $image_file_name);
        $stmt->execute();
        $stmt->close();
...
<div class="container mt-4 mb-4">
    <div class="main-title">Add Employee</div>
...
$(document).ready(function() {
    ...
    // If department is set from POST, pre-populate unit dropdown
    <?php if (isset($department_id) && $department_id > 0 && isset($unit_id) && $unit_id > 0 && !isset($errors['department_id']) && !isset($errors['unit_id'])) : ?>
    $.ajax({
        url: 'get_units.php',
        method: 'POST',
        ...
        success: function(data) {
            ...
            unitSelect.clearOptions();
            unitSelect.addOption({value:"",text:"--- Select Unit ---"});
            /* Returned Units are populating here ..  */
            $.each(data, function(i, unit){  
                unitSelect.addOption({value:unit.unit_id, text:unit.unit_name});
            });
            unitSelect.refreshOptions(false);
